...
void delete_cart(medicine *ptr, int no_sellers, char **sellers, int id, int quantity,
                medicine *ptr1,int cart_count,int new_cart_count) {
...
//register an account-create new file
...
void account_customer(void) {
  ...
  scanf("%s %s %s",person.name,person.email,person.phone);

  char fname[15];

  strcpy(fname,person.phone);
  ...
  fprintf(fp,"%s %s %s %s",person.name,person.email,person.phone,person.password);
...
int login_customer(char fname[]){
  ...
  fscanf(fp,"%s %s %s %s",person.name,person.email,person.phone,person.password);
